...
<img width="972" height="929" alt="image" src="https://github.com/user-attachments/assets/cb89f03b-0b26-4b25-bda0-c34c27785bae" />
...
Для проведения анализа использовались встроенные инструменты среды разработки IntelliJ IDEA, включая Inspect Code, Analyze Dependencies и Locate Duplicates. 

<img width="805" height="798" alt="image" src="https://github.com/user-attachments/assets/22fbaea7-0330-41f9-b3e6-31b7482f79b7" />

Статический анализ кода (Inspect Code) выявил 46 предупреждений, 4 слабых предупреждения и 134 орфографических ошибки. Основные замечания касаются устаревших вызовов printStackTrace(), неиспользуемых переменных и импортов, проблем с аннотациями null безопасности, а также избыточных конструкций, которые можно заменить аннотациями Lombok. 

<img width="974" height="412" alt="image" src="https://github.com/user-attachments/assets/24936243-0b10-4737-b5cb-a0d8d3604bca" />

Анализ связности и зависимостей показал, что проект организован по слоям: application, domain, presentation, security, config, и db.migration. Наиболее насыщенными по количеству классов являются пакеты user.command и user.query, что указывает на активное использование паттерна CQRS. Зависимости между слоями соблюдают принцип разделения ответственности, однако выявлены участки с избыточной связностью, особенно в контроллерах, где нарушается инкапсуляция логики.

<img width="974" height="347" alt="image" src="https://github.com/user-attachments/assets/1b231391-4e37-40b4-a830-1603925c4045" />

Анализ дублирования кода выявил повторяющиеся фрагменты в нескольких классах, включая ProfileController, TourCont, RegCont и AppUser. Основные дубликаты связаны с обработкой файлов, логикой сохранения данных и структурой DTO. Такое повторение усложняет сопровождение, увеличивает риск ошибок при изменениях и снижает читаемость проекта.
...
Unit-тестирование применялось для проверки корректности работы отдельных методов бизнес-логики, используемых в сервисах приложения онлайн-сервиса для парусных туров. Целью данного этапа было подтверждение того, что каждый модуль ведёт себя предсказуемо и возвращает корректные результаты при заданных входных данных. В процессе написания unit-тестов использовались JUnit 5 и Mockito, позволяющие изолировать тестируемые методы от внешних зависимостей и проверять их работу независимо от состояния сервера, базы данных или других сервисов. 
Тестированию подверглись ключевые функции, связанные с обработкой туров и бронирований: расчёт стоимости с учётом скидок, проверка корректности диапазона дат путешествия, определение количества свободных мест, формирование итоговой цены бронирования на нескольких участников и валидация параметров тура. Все тесты направлены на покрытие различных ветвей логики, включая позитивные и негативные сценарии, чтобы обеспечить стабильную работу приложения в реальных условиях.
1 Тест корректности расчёта стоимости тура с учётом скидки.

```@Test
void testCalculateTourFinalPrice() {
    TourService service = new TourService();

    double result = service.calculateFinalPrice(1000, 10); // 10% скидка
    assertEquals(900, result);
}
```
Данный тест проверяет корректность применения процентной скидки к базовой стоимости тура. Метод должен уменьшать итоговую стоимость на указанный процент, и тест подтверждает правильность вычислений на типичном примере.



2 Тест расчёта количества доступных мест на тур.
```
@Test
void testCalculateAvailableSlots() {
    AvailabilityService service = new AvailabilityService();

    int available = service.getAvailableSlots(12, 5);

    assertEquals(7, available);
}
```
Метод определяет количество оставшихся мест на тур исходя из общего количества мест и уже занятых слотов. Тест проверяет корректность логики определения доступности, что важно при отображении пользователям информации о турах и при оформлении бронирования.
3 Тест обработки некорректных параметров при бронировании.
```
@Test
void testInvalidPersonsThrowsException() {
    BookingCostService service = new BookingCostService();

    assertThrows(IllegalArgumentException.class, () ->
            service.calculateCost(1500, -2)
    );
}
```
Тест проверяет негативный сценарий: если пользователь указал отрицательное количество участников или ноль, метод обязан выбросить исключение. Это предотвращает некорректные операции в бизнес-логике и обеспечивает целостность данных.
4 Тест вычисления стоимости бронирования на группу участников.
```
@Test
void testCalculateGroupBookingCost() {
    BookingService booking = new BookingService();

    double cost = booking.calculateGroupBookingCost(300, 4);
    assertEquals(1200, cost);
}
```
Этот тест проверяет корректный расчёт итоговой стоимости бронирования в зависимости от количества участников. Метод применяется при оформлении групповых туров, и тест подтверждает корректность вычислений.
5 Тест проверки корректности диапазона дат тура.
```
@Test
void testValidateTourDates() {
    TourService service = new TourService();

    LocalDate start = LocalDate.of(2025, 6, 1);
    LocalDate end = LocalDate.of(2025, 6, 10);

    assertTrue(service.validateTourDates(start, end));
}
```
Этот тест проверяет логику валидации дат тура. Начальная дата должна быть раньше конечной, а длительность аренды яхты должна быть положительной. Метод используется при создании новых туров, чтобы исключить ошибочные даты.

### Интеграционные тесты

Интеграционное тестирование применялось для проверки взаимодействия разных слоёв приложения: REST-контроллеров, сервисов и репозиториев, а также корректности обработки запросов через HTTP. Для интеграционных тестов использовались Spring Boot Test, MockMvc.
Целью интеграционного тестирования было убедиться, что REST-эндпоинты корректно принимают параметры, вызывают необходимые методы сервисов и возвращают ожидаемый формат ответа. Тестировались как успешные сценарии (получение списка доступных туров, успешное создание бронирования), так и ошибочные ситуации (передача некорректных параметров).
1 Тест получения списка туров через API.
```
@SpringBootTest
...
```
Тест отправляет HTTP-запрос к endpoint /api/tours и проверяет структуру ответа. Это подтверждает, что контроллер корректно работает, возвращает список туров и данные проходят все слои приложения.
2 Тест успешного создания бронирования.
```
@SpringBootTest
@AutoConfigureMockMvc
...
```
Проверяется корректность цепочки: контроллер – сервис – расчёт стоимости – ответ API. Тест моделирует реальный запрос клиента на бронирование тура.
3 Тест ошибки при создании бронирования с некорректными данными.
```
@Test
void testCreateBookingInvalidPersons() throws Exception {
    mockMvc.perform(post("/api/booking")
            .param("tourId", "1")
            .param("persons", "0"))
...
```
Проверяется, что API корректно обрабатывает неверные входные данные и возвращает клиенту соответствующий статус ошибки.
4 Тест получения одного тура по ID.
```
@Test
void testGetSingleTour() throws Exception {
    mockMvc.perform(get("/api/tours/1"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$.id").value(1));
}
```
Этот тест проверяет работу маршрута получения конкретного тура по его идентификатору. Такой функционал является обязательным в бронировании и в подробном просмотре туров пользователем.
5 Тест фильтрации туров по максимальной цене.
```
@Test
void testFilterToursByPrice() throws Exception {
    mockMvc.perform(get("/api/tours/filter")
            .param("maxPrice", "500"))
            .andExpect(status().isOk())
            .andExpect(jsonPath("$[0].price").value(lessThanOrEqualTo(500)));
}
```
Проверяет работу эндпоинта, выполняющего фильтрацию туров по максимальной цене. Тест подтверждает корректность фильтрации на уровне контроллера, сервиса и репозитория.
